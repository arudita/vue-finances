      ...
      <table class="min-w-full">
        ...
        <tbody>
          <tr v-for="(product, index) in products" :key="index" class="border-t border-gray-100 dark:border-gray-800">
            ...
            <td class="py-3 whitespace-nowrap">
              <span :class="{ 'rounded-full px-2 py-0.5 text-theme-xs font-medium': true, 'bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500' : product.status === 'Delivered', 'bg-warning-50 text-warning-600 dark:bg-warning-500/15 dark:text-orange-400' : product.status === 'Pending', 'bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500' : product.status === 'Canceled' }">
                {{ product.status }}
              </span>
            </td>
          </tr>
